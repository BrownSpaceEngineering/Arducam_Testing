// This is a modified version of the original ArducamDemo.c file
// readDataThread has been modified to write the most recent frame to a file
// test_data in the current directory in addition to sending it over the network
#include <stdlib.h>
#include <stdio.h>
#include "ArduCAM.h"
#include "sccb_bus.h"
...
void *readDataThread(void *arg) {
  while (1) {
	...
	FILE *f = fopen("test_data", "w+");
	    fwrite(sendbuf_cam1, 1, length_cam1, f);
	        fclose(f);
      }
      else {
	      printf("bad\n");
